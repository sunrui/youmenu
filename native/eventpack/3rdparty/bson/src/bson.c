...
MONGO_EXPORT const bson *bson_shared_empty( void ) {
    static const bson shared_empty = { bson_shared_empty_data, bson_shared_empty_data, 128, 1, 0, 0, 0, 0, NULL, { 0 } };
    return &shared_empty;
}
...
MONGO_EXPORT void bson_print_raw( const char *data , int depth ) {
    ...
    while ( bson_iterator_next( &i ) ) {
        ...
        case BSON_NULL:
            ...
            break;
        case BSON_REGEX:
            bson_printf( "BSON_REGEX: %s\n", bson_iterator_regex( &i ) );
            break;
        case BSON_CODE:
            bson_printf( "BSON_CODE: %s\n", bson_iterator_code( &i ) );
            break;
